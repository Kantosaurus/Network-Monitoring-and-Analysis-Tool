        ...
        {Array.from({ length: max }).map((_, i) => {
          ...
          return (
            <React.Fragment key={i}>
              <div className={`p-2 rounded font-mono break-all ${same ? 'bg-white/5' : 'bg-red-600/10 border border-red-600/30'}`}>
                {l || <span className="opacity-30">∅</span>}
              </div>
              <div className={`p-2 rounded font-mono break-all ${same ? 'bg-white/5' : 'bg-green-600/10 border border-green-600/30'}`}>
                {r || <span className="opacity-30">∅</span>}
              </div>
            </React.Fragment>
          );
        })}
    ...
    return (
      <div className="space-y-1 text-xs font-mono">
        {Array.from({ length: max }).map((_, i) => {
          ...
          if (same) {
            return (
              <div key={i} className="p-2 bg-white/5 text-gray-500">
                  {l}
              </div>
            );
          }
          ...
          return (
            <React.Fragment key={i}>
              {l && (
                <div className="p-2 bg-red-600/10 border-l-2 border-red-600 text-red-600 break-all">
                  - {l}
                </div>
              )}
              {r && (
                <div className="p-2 bg-green-600/10 border-l-2 border-green-600 text-green-600 break-all">
                  + {r}
                </div>
              )}
            </React.Fragment>
    ...
    <div className="h-full flex flex-col gap-4">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-2">
          <label className="text-xs font-medium text-gray-700 dark:text-gray-300">View Mode:</label>
          <select value={viewMode} onChange={e => setViewMode(e.target.value as any)} className="glass-card dark:glass-card-dark rounded-lg border px-3 py-1 text-xs">
        ...
        <div className="flex items-center gap-4 text-xs">
          <div className="glass-card dark:glass-card-dark px-2 py-1 rounded">
            <span className="text-gray-500">Lines: </span>
            <span className={charsDiff > 0 ? 'text-blue-600' : 'text-green-600'}>
          ...
          {charsDiff > 0 && (
            <div className="glass-card dark:glass-card-dark px-2 py-1 rounded bg-blue-900/20 text-blue-600">
      ...
      {(!left || !right) && (
        <div className="grid grid-cols-2 gap-4 h-56">
          <div className="flex flex-col">
            <label className="text-xs font-medium text-gray-700 dark:text-gray-300 block mb-1">Left</label>
            <textarea
              value={left}
          ...
          <div className="flex flex-col">
            <label className="text-xs font-medium text-gray-700 dark:text-gray-300 block mb-1">Right</label>
            <textarea
              value={right}
